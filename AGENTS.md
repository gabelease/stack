...
- Never mutate configured trunk branches. By default these are `dev`, `main`,
  and `master`; repos can override them with `git config --add stack.trunk <branch>`
  or the temporary `STACK_TRUNKS` environment variable.
